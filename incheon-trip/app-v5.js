const PLACES={
 inspire:{name:"인스파이어 오로라",lat:37.465,lng:126.389,kind:"activity",naver:"https://map.naver.com/p/search/인스파이어%20오로라",time:"오후 · 낮잠 후 유동",desc:"1안 · 60–90분 · 26일 공연 혼잡 가능 · 핀은 대략 위치, 길찾기는 네이버 지도"},
 bakery:{name:"마시안제빵소",lat:37.434,lng:126.41,kind:"activity",naver:"https://map.naver.com/p/search/마시안제빵소",time:"오후 · 낮잠 후 유동",desc:"2안 · 45–60분 · 9/26 영업 확인 · 핀은 대략 위치, 길찾기는 네이버 지도"},
 mieum:{name:"카페 미음",lat:37.431,lng:126.414,kind:"activity",naver:"https://map.naver.com/p/search/카페%20미음",time:"오후 · 낮잠 후 유동",desc:"2안 카페 선택 · 9/26 영업 확인 · 핀은 대략 위치, 길찾기는 네이버 지도"},
 ...
 lunch:{name:"연경 차이나타운본점",lat:37.4756379,lng:126.619844,kind:"meal",naver:"https://naver.me/5K5Go1Ld",time:"11:30 전후 · 낮잠 시 뒤로",desc:"낮잠이 시작되면 식사 시간을 늦춤"},
 sinpo:{"name":"신포시장 포장 · 찬누리 기본 후보","lat":37.4716236,"lng":126.628167,"kind":"food","naver":"https://map.naver.com/p/entry/place/13582187","time":"점심 후 · 유동","desc":"시장 내 대략 핀 · 찬누리/야채치킨/치킨꼬꼬/타르트야 · 원조신포닭강정 9/26 휴무"},
 hanaro:{"name":"롯데마트 영종도점","lat":37.4925552,"lng":126.4910728,"kind":"shop","naver":"https://map.naver.com/p/entry/place/13028016","time":"오후 방문 후 · 장보기","desc":"9/26 10:00–22:30 · 9/27 휴무 · 인스파이어 → 마트 → 숙소"},
 stay:{name:"아빛스테이",lat:37.43285,lng:126.4172,kind:"stay",naver:"https://naver.me/GDa6kEs1",time:"오후 · 장보기 후 체크인",desc:"입실 후 자쿠지 물 받기 1.5–2시간 · 유아용품 신청 완료"},
 ...
 care:{"name":"뉴고려병원","lat":37.64085,"lng":126.66006,"kind":"meal","naver":"https://map.naver.com/p/search/뉴고려병원","time":"9/27 · 면회 시간 확인 후 결정","desc":"김포한강3로 283 · 방문지 변경 · 병문안 후 큰이모와 점심 선택"},
...
let activePlan="inspire", activeCafe="bakery";
const DAY1=["hospital","lunch","sinpo","inspire","hanaro","stay","masian"];
const DAY2=["jangmo","care"];
const BACKUP=["inspire","bakery","mieum"];
...
const stopLabels = {hospital:"1·2",lunch:"3",sinpo:"4",inspire:"5",bakery:"5",mieum:"5",hanaro:"6",stay:"7·8",masian:"9",jangmo:"1",care:"2·3"};
...
function initMap() {
  ...
  [...new Set([...DAY1,...DAY2,...BACKUP])].forEach(pid=>{
    const p=PLACES[pid];
...
function showLayer(layer) {
  ...
  if (layer === "backup") BACKUP.filter(k=>!DAY1.includes(k)).forEach(k=>backupMarkers[k].addTo(map));
  if (routeLine && layer !== "day2") routeLine.addTo(map);
  document.getElementById("mapNote").textContent=layer === "day2" ? "DAY 2 · 아침 → 뉴고려병원 → 큰이모와 점심 선택 · 시간 미정" : layer === "backup" ? "1안 인스파이어 · 2안 마시안제빵소 / 미음 · 새 핀은 대략 위치" : "일정과 같은 번호 · 선은 방문 순서 · 새 핀은 대략 위치";
  fitCurrentLayer();
}
...
function focusStop(pid,fromMap=false,selector=null) {
  ...
  if (!map) return;
  const layer=DAY1.includes(pid)?"day1":DAY2.includes(pid)?"day2":"backup";
  if (currentLayer!==layer) showLayer(layer);
  const marker=DAY1.includes(pid)||DAY2.includes(pid)?markers[pid]:backupMarkers[pid];
  if (!marker) return;
...
function selectPlan(plan, cafe=activeCafe) {
 activePlan=plan;activeCafe=cafe;
 const pid=plan==="inspire"?"inspire":cafe;
 DAY1[3]=pid;
 document.querySelectorAll("[data-plan]").forEach(b=>{b.classList.toggle("active",b.dataset.plan===plan);b.setAttribute("aria-pressed",b.dataset.plan===plan)});
 document.getElementById("cafeChoice").hidden=plan!=="cafe";
 document.getElementById("s-activity").dataset.place=pid;
 document.getElementById("activityTitle").textContent=(plan==="inspire"?"1안 · ":"2안 · ")+PLACES[pid].name;
 document.getElementById("activityNaver").href=PLACES[pid].naver;
 document.getElementById("activityNote").textContent=plan==="inspire"?"영종도 이동 후 60–90분 관람. 14:10 한복 퍼레이드는 시간이 맞을 때만. 26일 아레나 공연 혼잡 가능. 주차 조건 현장 확인.":"영종도 이동 후 45–60분 휴식. 두 카페 모두 9/26 영업·아기의자 확인. 비가 그치면 해변 짧게 산책.";
 document.getElementById("planSummary").textContent="점심·신포시장 포장 → 영종도 → "+PLACES[pid].name+" → 롯데마트 영종도점 → 아빛스테이";
 try{localStorage.setItem("incheon-plan-v9",JSON.stringify({plan,cafe}))}catch(e){}
 if(map){drawRoadRoute();showLayer(currentLayer)}
 else if(document.querySelector(".map-fallback"))initMap();
 window.dispatchEvent(new CustomEvent("trip-plan-change",{detail:{pid,plan}}));
}
document.querySelectorAll("[data-plan]").forEach(b=>b.addEventListener("click",()=>selectPlan(b.dataset.plan)));
document.getElementById("cafeSelect").addEventListener("change",e=>selectPlan("cafe",e.target.value));
try{const saved=JSON.parse(localStorage.getItem("incheon-plan-v9"));if(saved&&["inspire","cafe"].includes(saved.plan)&&["bakery","mieum"].includes(saved.cafe)){document.getElementById("cafeSelect").value=saved.cafe;selectPlan(saved.plan,saved.cafe)}}catch(e){}
const napText={sleep:"차에서 바로 잠들면 점심 때문에 깨우지 않고 평소 약 2시간 낮잠을 우선. 오후 관람을 줄여도 괜찮아요.",awake:"안 잠들면 11:30 전후 점심 → 신포시장 짧게 → 12시대 차 이동을 낮잠 구간으로 사용. 영종도 방문은 늦춰도 됨.",short:"15–20분만 자면 가장 애매합니다. 오후 방문을 늦추더라도 추가 수면을 먼저 시도."};document.querySelectorAll("#napSwitch button").forEach(b=>b.addEventListener("click",()=>{document.querySelectorAll("#napSwitch button").forEach(x=>x.classList.remove("active"));b.classList.add("active");document.getElementById("napAdvice").textContent=napText[b.dataset.nap];document.querySelectorAll("#napSwitch button").forEach(x=>x.setAttribute("aria-pressed",x===b))}));
const PACK=["기저귀 넉넉히 + 여분 2–3개","물티슈 · 휴대티슈 · 기저귀갈이 매트","기저귀 크림 · 바디워시, 로션, 선크림","여벌옷 2세트 이상 · 잠옷 · 양말","면회 전 버거킹 햄버거 약 4개 픽업","턱받이 · 아기 수저/포크 · 빨대컵","물 · 이동용 간식 · 잉글리시 에그 여행용, 아이패드, 에그펜, 작은 장난감","아기 식사 백업 — 즉석밥·김·평소 먹는 반찬","유모차","아기띠 · 얇은 담요","애착담요 · 인형(토끼), 그림책","체온계 · 상비약","모자","수영복 · 방수기저귀","부모 여벌옷 · 폼클렌징, 치약칫솔","충전기 · 보조배터리","신분증/지갑"];
const BBQ=["집에서 · 양고기 프렌치랙(구매 완료) · 소시지","구울 채소 · 새송이버섯 · 양파 · 통마늘","청양고추","양념 · 와사비 · 허브솔트 · 고기용 소금 · 머스타드","김치 · 선택: 명이나물/양파절임","햇반 · 라면","나윤이 저녁 · 밥 · 고기 · 포켓미역국, 파우치 소스","집에서 · 소주 · 맥주, 파인애플 소화제","보냉가방 · 아이스팩","마트 · 삼겹살 또는 목살(선택)","마트 · 생수 · 음료 · 선택: 파인애플"];
...
try{if(!localStorage.getItem("incheon-check-migration-v9")){["pack-4","bbq-11","bbq-12"].forEach(id=>localStorage.removeItem("incheon-v5:"+id));localStorage.setItem("incheon-check-migration-v9","1")}}catch(e){}
try{if(!localStorage.getItem("incheon-check-migration-v11")){["bbq-0","bbq-2","bbq-3","bbq-4","bbq-5","bbq-6","bbq-7","bbq-8","bbq-9","bbq-10","bbq-11","bbq-12"].forEach(id=>localStorage.removeItem("incheon-v5:"+id));localStorage.setItem("incheon-check-migration-v11","1")}}catch(e){}
try{if(!localStorage.getItem("incheon-check-migration-v12")){const oldLists={"PACK":["기저귀 넉넉히 + 여분 2–3개","물티슈 · 휴대티슈 · 기저귀갈이 매트","기저귀 크림 · 기저귀 봉투/비닐","여벌옷 2세트 이상 · 잠옷 · 양말","면회 전 버거킹 햄버거 약 4개 픽업","턱받이 · 아기 수저/포크 · 빨대컵","물 · 이동용 간식 · 과일/퓨레","아기 식사 백업 — 즉석밥·김·평소 먹는 반찬","유모차 · 레인커버","아기띠(필요 시) · 얇은 담요","수면 애착템 · 취침 루틴 용품","체온계 · 평소 쓰는 상비약","선크림 · 모자","수영복 · 방수기저귀 · 아쿠아슈즈","부모 여벌옷 · 세면도구","충전기 · 보조배터리 · 차량 충전선","우산/우비 · 얇은 겉옷","신분증/지갑 · 면회 관련 필요물품","숙소 예약정보/체크인 연락처 캡처"],"BBQ":["집에서 · 양고기 프렌치랙(구매 완료) · 소시지","나윤이용 간하지 않은 고기/식사 별도","구울 채소 · 새송이버섯 · 양파 · 통마늘","선택 · 아스파라거스 또는 꽈리고추","양념 · 와사비 · 허브솔트 · 고기용 소금 · 머스타드","김치 · 선택: 명이나물/양파절임","햇반 · 라면","나윤이 저녁 · 밥 · 고기 · 포켓미역국","집에서 · 소주 · 맥주","보냉가방 · 아이스팩","회타운 회 포장 · 소스 포함 여부 확인","BBQ 사전예약 여부 확인 · LPG 2만원","객실 내 고기·해산물 조리 및 개인 화구 금지","키친타월 · 호일 · 랩 · 지퍼백 필요 여부","쓰레기 분리배출 방식 확인","마트 · 삼겹살 또는 목살(선택)","마트 · 생수 · 음료 · 선택: 파인애플"]},newLists={"PACK":["기저귀 넉넉히 + 여분 2–3개","물티슈 · 휴대티슈 · 기저귀갈이 매트","기저귀 크림 · 바디워시, 로션, 선크림","여벌옷 2세트 이상 · 잠옷 · 양말","면회 전 버거킹 햄버거 약 4개 픽업","턱받이 · 아기 수저/포크 · 빨대컵","물 · 이동용 간식 · 잉글리시 에그 여행용, 아이패드, 에그펜, 작은 장난감","아기 식사 백업 — 즉석밥·김·평소 먹는 반찬","유모차","아기띠 · 얇은 담요","애착담요 · 인형(토끼), 그림책","체온계 · 상비약","모자","수영복 · 방수기저귀","부모 여벌옷 · 폼클렌징, 치약칫솔","충전기 · 보조배터리","신분증/지갑"],"BBQ":["집에서 · 양고기 프렌치랙(구매 완료) · 소시지","구울 채소 · 새송이버섯 · 양파 · 통마늘","청양고추","양념 · 와사비 · 허브솔트 · 고기용 소금 · 머스타드","김치 · 선택: 명이나물/양파절임","햇반 · 라면","나윤이 저녁 · 밥 · 고기 · 포켓미역국, 파우치 소스","집에서 · 소주 · 맥주, 파인애플 소화제","보냉가방 · 아이스팩","마트 · 삼겹살 또는 목살(선택)","마트 · 생수 · 음료 · 선택: 파인애플"]};for(const name of ["PACK","BBQ"]){const prefix=name==="PACK"?"pack":"bbq",saved=oldLists[name].map((_,i)=>localStorage.getItem("incheon-v5:"+prefix+"-"+i));newLists[name].forEach((label,i)=>{const prior=oldLists[name].indexOf(label),key="incheon-v5:"+prefix+"-"+i;if(prior>=0&&saved[prior]!==null)localStorage.setItem(key,saved[prior]);else localStorage.removeItem(key);});}localStorage.setItem("incheon-v5:call-stay","1");localStorage.setItem("incheon-check-migration-v12","1");}}catch(e){}
...
document.querySelectorAll("[data-call-note]").forEach(el=>{const key="incheon-call-note:"+el.dataset.callNote;try{el.value=localStorage.getItem(key)||""}catch(e){}el.addEventListener("input",()=>{try{localStorage.setItem(key,el.value)}catch(e){}})});

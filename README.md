...
`/research/` 페이지의 연구 내용과 분석 단계는 `_data/research.yml`에서
관리합니다. 각 연구는 `status`, `version`, `reports` 목록으로 완료 여부와
버전별·언어별 보고서를 구분합니다. 인디언 포커는 완료된 2023년 졸업연구이고,
다빈치코드 v2.0은 후속 검증과 모형 확장이 예정된 진행 중 기술연구입니다.
아직 공개하지 않은 연구 주제나 보고서에서 검증되지 않은 결과는 추가하지 않습니다.
